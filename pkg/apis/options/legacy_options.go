...
type LegacyHeaders struct {
	...
	AuthMethod              string `flag:"auth-method" cfg:"auth_method"`
}
...
func legacyHeadersFlagSet() *pflag.FlagSet {
	...
	flagSet.String("auth-method", "AuthMethod", "The name of the authentication method header. Auth methods are [cookie, header].")
...
func (l *LegacyHeaders) getRequestHeaders(csrfHeader string) []Header {
	...
	if l.AuthMethod != "" {
		requestHeaders = append(requestHeaders, getAuthMethodHeader(l.AuthMethod))
	}
...
func (l *LegacyHeaders) getResponseHeaders() []Header {
	...
	if l.AuthMethod != "" {
		responseHeaders = append(responseHeaders, getAuthMethodHeader(l.AuthMethod))
	}
